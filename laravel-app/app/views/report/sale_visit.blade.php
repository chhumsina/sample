@section('title', 'Sale Report Visit')
@section('content')
<?php
$baseUrl = URL::to('/');
$khanCode = '';
	...
	<div class="col-md-12">
		...
		<div class="panel panel-default">
		  <div class="panel-heading">
		  	<div class="row">
				{{ Form::open(array('url' => 'reports/sale_visit')) }}
		  	 		<div class="row">
		  	 			<div class="col-md-2">
	  	 			...
	  	 			<div class="row">
	  	 				...
	  	 				<div class="col-md-2">
	  	 					<?php $staffId = ''; if (Input::has('sale__id')){$staffId = Input::get('sale__id');} ?>
	  	 					{{Form::text('sale__id',$staffId)}}
	  	 				</div>
	  	 				<div class="col-md-2">
	  	 			...
	  	 			<div class="row">
	  	 				...
	  	 				<div class="col-md-2">
	  	 					...
	  	 					{{Form::text('dealer__id',$dealerId)}}
	  	 				</div>
	  	 				<div class="col-md-2">
	  	 					<label>Dealer Name:</label>
	  	 				</div>
	  	 				<div class="col-md-2">
				  	 	    <?php $dealerName = ''; if (Input::has('dealer_name')){$dealerName = Input::get('dealer_name');}?>
				  	 	    {{Form::text('dealer_name',$dealerName)}}
	  	 				</div>
	  	 			</div>
	  	 			<div class="row">
		  	 			<div class="col-md-2">
		...
		@if ($saleVisits->count())
		    <table class="table table-striped table-bordered">
		        ...
		        <tbody>
		        	<?php $i = ($saleVisits->getCurrentPage() - 1)* $saleVisits->getPerPage(); ?>
		            @foreach ($saleVisits as $saleVisit)
		            	<?php $i += 1; ?>
		                <tr>
		                    <td>{{ $i }}</td>
		                    <td>{{ $saleVisit->id }}</td>
		          			<td>{{ $saleVisit->name }}</td>
		          			<td>{{ $saleVisit->phone }}</td>
		          			<td>{{ $saleVisit->dealer__id }}</td>
		          			<td>{{ $saleVisit->dealerName }}</td>
		          			<td>{{ $saleVisit->dealerPhone }}</td>
		          			<td>{{ $saleVisit->provinceName }}</td>
		          			<td>{{ $saleVisit->khanName }}</td>
		          			<td>{{ $saleVisit->terminal__serial }}</td>
		          			<td>{{ $saleVisit->datetime }}</td>
		          		</tr>
		          	@endforeach
		        </tbody>
		    </table>
			<?php echo $saleVisits->links(); ?>
		@else
		    There are no Sale Visit!
		@endif
		</div>
		@stop
	</div>
